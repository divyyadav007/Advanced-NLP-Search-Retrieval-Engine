...
class RAGMetricsRunner:
    ...
    def __init__(
        self, retriever: HybridRetriever, reranker: DocumentReranker, generator: GroundedGenerator
    ):
    ...
    def run_batch_evaluation(self, test_dataset: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        for idx, test_case in enumerate(test_dataset):
            ...
            try:
                ...
                elite_chunks = self.reranker.rerank(
                    query, hybrid_candidates, top_n=config.RERANK_TOP_N
                )
